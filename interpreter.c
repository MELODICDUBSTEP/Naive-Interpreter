...
//a sequence of command implemented by linked list

struct res_prog {
  struct cmd * foc;//focused program
  struct cont_list * ectx; //evaluation context
};

struct SLL_hash_table * var_state;
// the variable name mapping to the value
...
//a function that creates a new residual program
...
// to add a new command to the continuation list
...
//to initialize the residual program
...
int test_end(struct res_prog * r) {
  ...
  //if both the focused program and the evaluation context 
  //are empty then the program ends.
  else {
    return 0;
  }
}
